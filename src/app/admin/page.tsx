...
async function getStats() {
    const [totalPosts, publishedPosts, totalProjects, draftProjects] = await Promise.all([
        prisma.post.count(),
                ...
                <div className='flex flex-col gap-0'>
                    {recentPosts.length === 0 ? (
                        <p className='text-sm text-gray-600 py-4 text-center'>
                            No post yet. {' '}
                            <Link href='/admin/posts/new' className='text-lime-400 hover:underline'>
                            Write your first post <ArrowRight />
                            </Link>
                        </p>
                    ): (
                        recentPosts.map((post) => (
                            <div key={post.id} className='flex items-center justify-between py-3 border-b border-gray-800 last:border-0'>
                                <span className='text-sm text-gray-200 font-medium truncate max-w-sm'>
                                    {post.title}
                                </span>
                                <div className='flex items-center gap-3 shrink-0'>
                                    <span className={`font-mono text-xs px-2 py-0.5 rounded border ${post.status === 'published' ? 'border-lime-400/30 text-lime-400 bg-lime-400/5' : 'border-gray-700 text-gray-500 bg-gray-800'}`}>
                                    {post.status}
                                    </span>
